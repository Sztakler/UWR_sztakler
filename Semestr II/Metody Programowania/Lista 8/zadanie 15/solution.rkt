...
(struct thunk (e env) #:transparent)  ;zadanie 15
...
(define (eval-env e env)
  (match e
    ...
    [(thunk e env) (eval-env e env)]                    ;zadanie 15 (start podpunktu)
    [(var-expr x) (let ([ex (env-lookup x env)]) 
                    (match ex
                      [(thunk ex env) (eval-env ex env)]
                      [_ ex]))]                         ;(koniec podpunktu)
    [(if-expr eb et ef) (if (eval-env eb env)
                            (eval-env et env)
                            (eval-env ef env))]
    [(cons-expr e1 e2) (cons (thunk e1 env)             ;zadanie 15 (startowanko consowanka)
                             (thunk e2 env))]
    [(car-expr e) (eval-env (car (eval-env e env)) env)]
    [(cdr-expr e) (eval-env (cdr (eval-env e env)) env)] ;(kończońsko)
    [(null-expr) null]
...
;TEŚCIORY
;(eval (parse'((lambda (x) (+ 3 3)) (/ 5 0))))                                     ;6
;(eval (parse'(let [if-fun (lambda (b t e) (if b t e))](if-fun  true 4 (/ 5 0))))) ;4
;(eval (parse' (car (cons (+ 2 2) (/ 5 0)))))                                      ;4
