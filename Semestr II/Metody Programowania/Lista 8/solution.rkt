...
;Zadanie wykonane we współpracy z Bartoszem Janikowskim, po konsultacji z Cezarym Świtałą
...
(struct apply-expr (f xs)     #:transparent) ; zadanie 14
...
(define (expr? e)
  (match e
    ...
    [(apply-expr f xs)      (and (expr? f) (expr? xs))] ;zadanie 14
    [_ false]))
...
(define (parse q)
  (cond
    ...
    [(and (list? q) (eq? (first q) 'list))               ; dodana obsługa list z zad 7
    ...
    [(and (list? q) (eq? (length q) 3) (eq? (first q) 'let))
     (let-expr (first (second q))
               ...
               (parse (third q)))]
    [(and (list? q) (eq? (length q) 3) (eq? (first q) 'apply)) ;zadanie 14
     (apply-expr (parse (second q)) (parse (third q)))]           ;zadanie 14
...
(define (eval-env e env)
  (match e
    ...
    [(apply-expr f xs)                                                                               ;zadanie 14 (tu się zaczyna)
     (let ([vf (eval-env f env)]                                                                     
           [vxs (eval-env xs env)])                                                                  
       (match vf                                                                                     
         [(clo x body fun-env)
          (cond
            [(null? (cdr vxs)) (eval-env body (env-add x (car vxs) fun-env))]
            [else (eval-env (apply-expr body (cdr-expr xs)) (env-add x (car vxs) fun-env))])]
         [_ (error "ERROR")]))]))                                                                    ;a tu się kończy
...
(eval (parse
 '(apply (lambda (x y) (+ x y))
        (list 1 2))))                                                  ;3
(eval (parse'(apply (lambda (x y z) (+ x (+ y z)))(list 1 2))))               ;#<clo>
(eval (parse'(apply (lambda (x y) (lambda (z) (+ x (+ y z))))(list 1 2 3))))  ;6
;(eval (parse'(apply (lambda (x y) (+ x y))(list 1 2 3))))                     ;ERROR