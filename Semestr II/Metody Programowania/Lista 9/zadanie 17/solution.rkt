...
(define (op->proc op)
  (match op ['+ +] ['- -] ['* *] ['/ /] ['% modulo]
            ...
            ['eq? eq?]
            [_ false]))
;ZMODYFIKOWANA PROCEDURA (dodano drugie środowisko env-while)
...
(define (eval e) (eval-env e env-empty env-empty)) ;dodano drugie, puste środowisko
...
(define (eval-while e env-while) ;również modyfikacja, należało uzupełnić eval-env o dodatkowe środowisko
  (match e
    [(skip) env-while]
    [(assign x e)
